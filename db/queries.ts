...
export const getCourseProgress = cache(async () => {
    ...
    const unitsInActiveCourse = await db.query.units.findMany({
        orderBy: (units, {asc}) => [asc(units.order)], // sort in ascending order
        where: eq(units.courseId, userProgress.activeCourseId), // units active courses
        with: { // related lessons
            lessons: {
    ...
    const firstUncompletedLesson = unitsInActiveCourse
        ...
        .find((lesson) => {
            // todo: if smth dont work check the last if clause
